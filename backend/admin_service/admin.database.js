...
async function blogApproval(motel_id) {
  return new Promise((resolve, reject) => {
    connection.query(
      `UPDATE motel SET status = 1 WHERE motel_id = '${motel_id}'`,
      (error, result) => {
        if (error) reject(error);
        resolve(result);
      }
...
async function blogReject(motel_id) {
  return new Promise((resolve, reject) => {
    connection.query(
      `UPDATE motel SET status = -1 WHERE motel_id = '${motel_id}'`,
      (error, result) => {
...
async function acceptOwner(user_id) {
  return new Promise((resolve, reject) => {
    connection.query(
      `UPDATE user SET isWaitingForVerify = 0 AND role = 'owner' WHERE user_id = '${user_id}'`,
      (error, result) => {
        if (error) reject(error);
        resolve(result);
      }
...
async function declineOwner(user_id) {
  return new Promise((resolve, reject) => {
    connection.query(
      `UPDATE user SET isWaitingForVerify = 0 WHERE user_id = '${user_id}'`,
      (error, result) => {
        if (error) reject(error);
        resolve(result);
      }
